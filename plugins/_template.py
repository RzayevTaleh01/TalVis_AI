"""
Drop-in TalVis plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — TalVis discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    player: the TalVisUI instance — use player.write_log(f"TalVis: ...") to log,
    ...
    if player:
        try:
            player.write_log(f"TalVis: {result_text}")
        except Exception:
            pass
    return result_text
