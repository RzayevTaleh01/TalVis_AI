"""
dashboard/server.py — TalVis Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "TalVis Uploads",
        Path.home() / "Documents" / "TalVis Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'TalVis-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"TalVis Dashboard Port {port}"
        prog_rule  = "TalVis Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="talvis_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until TalVis is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "talvis.key").exists() and (certs / "talvis.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in TalVis to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('talvis_token','{tok}');
  sessionStorage.setItem('talvis_key','{key}');
  localStorage.setItem('talvis_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to TalVis…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "talvis.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "talvis.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in TalVis UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
