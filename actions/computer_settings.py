...
def _names_the_assistant(target: str) -> bool:
    """True when the close target IS the assistant rather than some other app.

    The name is user-configurable, so it is read from config rather than
    hardcoded. "talvis" stays recognised whatever the assistant is renamed to —
    people keep calling it that, and a shutdown request that goes unrecognised
    turns into a confusing "nothing to close".
    """
    t = re.sub(r"[^a-z0-9]+", "", (target or "").lower())
    if not t:
        return False
    names = {"talvis"}
    try:
        from memory.config_manager import get_assistant_name
        configured = re.sub(r"[^a-z0-9]+", "", (get_assistant_name() or "").lower())
        if configured:
...
def close_app():
    """Close the FOCUSED window — the no-target fallback only.

    Reaching this without an app name is a bug in the caller: computer_settings
    routes 'close_app' with a value to _close_app.close_by_name() long before
    ACTION_MAP is consulted. This used to be the whole implementation, and
    because the dispatcher calls every mapped action with no arguments, the app
    name the model supplied was silently dropped and Alt+F4 closed whatever had
    focus — most often TalVis's own HUD.
    """
...
_IRREVERSIBLE = {
    ...
    "toggle_wifi": ("Switch WiFi off or on",
                    "If this switches WiFi off, TalVis loses its connection and "
...
def computer_settings(
    ...
    # ── Closing a named app ──────────────────────────────────────────────────
    # Handled here rather than through ACTION_MAP because that path calls every
    # action as func() with no arguments, which is exactly how the app name got
    # thrown away and Alt+F4 ended up closing TalVis itself.
    if action in ("close_app", "quit_app", "exit_app"):
        target = str(value or params.get("app_name") or params.get("app") or "").strip()
        if not target:
            return ("No app was named. Ask the user which app to close — "
                    "do not close anything, and do NOT shut yourself down.")

        # "Close TalVis" is the one close request that means the assistant, and
        # it is the only way to end the session by voice. Routed to
        # shutdown_talvis rather than handled here so the goodbye and the
        # session summary still happen. A bare "close" never reaches this.
        if _names_the_assistant(target):
            return ("[SHUTDOWN_REQUESTED] The user named the assistant itself. "
                    "Call shutdown_talvis now.")

        try:
            from . import _close_app
        except Exception as e:
            return f"Cannot close apps right now: {e}"
        return _close_app.close_by_name(target)
        ...
        "properties": {
            ...
            "action": {
                ...
                "description": (
                    ...
                    # close_app needs the app's name in `value` ("Brave",
                    # "Spotify"). Without it nothing is closed — it will not
                    # fall back to closing the focused window, because that
                    # window is usually TalVis itself. close_window is the
            ...
            "value": {
                "type": "STRING",
                "description": (
                    "Optional value: volume level 0-100, text to type, key name, "
                    "or — for close_app — the name of the app to close ('Brave', "
                    "'Spotify'), which is REQUIRED for that action."
                )
            }
        },
        "required": []
