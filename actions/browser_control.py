...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".talvis_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".talvis_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using TalVis profile")
                talvis = str(Path.home() / ".talvis_profiles" / "firefox_talvis")
                Path(talvis).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(talvis, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".talvis_profiles" / "safari")
        ...
        # automation). Fall back to a persistent TalVis automation profile —
        # accounts logged in here once stay logged in on later sessions too.
        talvis_profile = str(Path.home() / ".talvis_profiles" / self.browser_name)
        Path(talvis_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with TalVis profile: {talvis_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(talvis_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with TalVis profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "talvis_screenshot.png")
