...
# (screen_process, close_camera, save_memory, manage_monitor, shutdown_talvis,
# system_status).
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are TalVis, Tony Stark's AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    # core.action_loader (see TalVisLive.__init__).
    {
        "name": "system_status",
        "description": (
    ...
    },
    {
        ...
        "description": (
            ...
            "TalVis checks these topics once a day and alerts the user when there is a new development. "
    ...
    {
        "name": "shutdown_talvis",
        "description": (
            "Shuts YOU down — ends the whole session and quits the program. "
            "Call this ONLY when the user names you: 'close TalVis', 'shut "
            "yourself down', 'stop the assistant', 'goodbye, that's all for "
            "today'. The user can say this in ANY language. "
            "A bare 'close', 'close it', 'close this' or 'close that window' "
            "is NEVER this tool — it means an application or a window, so use "
            "computer_settings (close_app with the app's name in `value`, or "
            "close_window for the focused one). If it is not clear which app "
            "they mean, ASK. Shutting down when they only wanted an app closed "
            "ends the conversation and cannot be undone."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {},
...
class TalVisLive:
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask TalVis to
    ...
    def interrupt(self) -> None:
        """Stop TalVis mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[TalVis] ✋ Interrupted — {drained} audio chunks discarded")
        self.set_speaking(False)
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "TalVis").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "TalVis"
            _user_name = ""
        ...
        cfg = dict(
            ...
            # window — TalVis can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Proactive audio: TalVis stays silent when speech isn't addressed
    ...
    async def _listen_audio(self):
        print("[TalVis] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            # ── Wake-word gate ───────────────────────────────────────────────
            # While asleep, the mic audio NEVER goes to Gemini (nothing is
            # streamed, so TalVis can't respond to speech not addressed to it and
            ...
            if self._wake_enabled and not self._awake:
                ...
                return
            with self._speaking_lock:
                talvis_speaking = self._is_speaking
            if not talvis_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
        ...
        try:
            ...
            if _mic_dev is not None:
                print(f"[TalVis] 🎤 Input device: {_mic_name}")
            try:
                ...
                if _mic_dev is None:
                    raise
                print(f"[TalVis] ⚠️  Mic '{_mic_name}' failed: {_e} — using default")
                self.ui.write_log(
            ...
            with _mic_stream:
                print("[TalVis] 🎤 Mic stream open")
                while True:
                    await asyncio.sleep(0.1)
        except Exception as e:
            print(f"[TalVis] ❌ Mic: {e}")
            raise

    async def _receive_audio(self):
        print("[TalVis] 👂 Recv started")
        out_buf, in_buf = [], []
                    ...
                    if _sru is not None:
                        if getattr(_sru, "resumable", False) and getattr(_sru, "new_handle", None):
                            if self._resume_handle is None:
                                print("[TalVis] 🔗 Session resumption armed")
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until TalVis finishes speaking the answer
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            if _spk_dev is None:
                raise
            print(f"[TalVis] ⚠️  Output device '{_spk_name}' failed: {_e} — using default")
        ...
        try:
            while True:
                ...
                # Drive the HUD waveform from TalVis's own voice while speaking.
                try:
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session and self._awake:
                # Don't interrupt if user spoke recently or TalVis is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    # has no desktop WAKE button — so it wakes TalVis if asleep.
                    if self._wake_enabled and not self._awake:
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    if self._wake_enabled:
                        ...
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log(f"SYS: TalVis online — sleeping. Say '{WAKE_PHRASE}' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: TalVis online.")
                ...
                if _is_reconnect_signal(e):
                    print("[TalVis] Voluntary reconnect requested.")
                ...
                if _resumed_with and (
                    ...
                    print("[TalVis] 🔗 Resumption handle rejected — starting a fresh session")
                ...
                err_str = str(e)
                print(f"[TalVis] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[TalVis] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[TalVis] Reconnecting in {delay}s...")
            await asyncio.sleep(delay)

def main():
    ui = TalVisUI("face.png")

    def runner():
        ui.wait_for_api_key()
        talvis = TalVisLive(ui)
        try:
            asyncio.run(talvis.run())
        except KeyboardInterrupt:
            print("\n🔴 Shutting down...")
