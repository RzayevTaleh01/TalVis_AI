...
class HudCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        # TalVis's own voice while speaking). Falls back to a gentle idle
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "talvis"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("talvis:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for TalVis", str(Path.home()),
            "All Files (*.*);;"
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="TalVis", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "TalVis"
...
class AudioDeviceOverlay(_HudOverlay):
    """Choose which microphone TalVis listens to and which speakers it uses.
    ...
    is plugged in. 'TalVis can't hear me' is usually 'TalVis is listening to the
    webcam'."""
    ...
    def __init__(self, parent=None):
        ...
        self._in_box  = _row("MICROPHONE — what TalVis hears you with",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("SPEAKERS — what TalVis talks through",
...
class MemoryOverlay(_HudOverlay):
    """Everything TalVis has stored about you, and when it learned it.
    ...
    def _rebuild(self):
        ...
        hdr = QLabel("🧠  WHAT TalVis REMEMBERS")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick TalVis actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — TalVis ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "TalVis").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop TalVis mid-speech
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _build_talvis_icon(out_path: Path) -> bool:
        """
        Render a TalVis arc-reactor icon at 4× resolution and downsample
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "talvis.ico"
        if not ico_path.exists():
            self._build_talvis_icon(ico_path)
    ...
    def _build_header(self) -> QWidget:
        ...
        _sub_text = ("Just A Rather Very Intelligent System"
                     if _disp in ("TalVis", "J.A.R.V.I.S")
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "TalVis_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.talvis.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "talvis.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "TalVis_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "TalVis_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.talvis.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.talvis.assistant</string>\n'
    ...
    def _wake_state(self) -> dict:
        ...
        if self.wake_get_state:
            try:
                ...
                pass
        # Before TalVisLive has wired its callback (drawer built at startup).
    ...
    def _toggle_wake_word(self):
        ...
        if not st["ready"]:
            ...
            return
        # Already downloaded → just flip enabled/disabled through TalVisLive.
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "TalVis") or "TalVis",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        self._assistant_name = name.strip() or "TalVis"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — {APP_VERSION}")
        self._title_lbl.setText(display)
        if display in ("TalVis", "J.A.R.V.I.S"):
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "TalVis") or "TalVis"
